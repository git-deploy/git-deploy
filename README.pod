...
Note: Your life will be much easier if you choose to implement sync hooks
instead.

If you choose to implement sync hooks for rollouts, but a sync hook fails (e.g.
because a gypsy ate your pigeon), git-deploy exits with an error and expects to
...
When you inevitably fuck up a rollout and you want to roll back as fast
...
B<git-deploy> makes it easy to see exactly where you went wrong, and
B<git-deploy show> will show you which of the several tags you've been jumping
back and forth between actually correspond to the same revision.
...
FIXME?
One thing it definitely doesn't do is worry about how your code
...
Note: this command used to be named "rollback", but this caused too much confusion.
...
The current phase can be obtained by inspecting the environment variable
$GIT_DEPLOY_PHASE (e.g. 'post-pull', 'pre-sync').

The current hook prefix can be obtained by inspecting the environment variable
$GIT_DEPLOY_HOOK_PREFIX (e.g. 'common', 'sheep').
