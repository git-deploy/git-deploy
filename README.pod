...
something that took hours to being so easy that deploying times a day
is what happens on a slow day.
...
Once you're happy with the state of things you want to sync out you
tell git-deploy, which will create a tag to record in history that the
current state is what you released.

If your current server is the only server you have you do a release at
this point:

    git-deploy release

That'll create a tag and push it, but it won't sync your code anywhere
else. If you have a bigger setup than that you do:

    git-deploy sync

Which will create a tag and sync it out. How the "sync it out" part
happens is B<completely up to you> (see L</Sync Hooks>), you can use
everything from git itself to carrier pigeons, git-deploy doesn't
care.

If the sync hook fails (e.g. because a gypsy ate your pigeon)
git-deploy exits with an error and expects to you fix the
situation.
